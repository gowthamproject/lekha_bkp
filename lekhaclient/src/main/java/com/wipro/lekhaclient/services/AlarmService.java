			...
			for (Nodes node : nodes) {
				...
				for (Integer nodeId : nodeIds) { 
					String responseJson = pullData(
							Constants.LEKHA_ENDPOINT + "/nms/events/" + region_id + "/" + nodeId, Constants.HTTP_METHOD_GET);
					if (responseJson != null && !responseJson.isEmpty()) {
						System.out.println("Alarm response ----:"+ responseJson);
						...
						for(AlarmInfo alarmInfo : alarmInfos) {
							...
							alarmList.forEach(p->p.setNode_name(null));
