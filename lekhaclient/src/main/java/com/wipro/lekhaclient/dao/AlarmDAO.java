...
public class AlarmDAO implements DAOInterface<Alarm> {
	...
	public void insertRecord(Alarm data) throws SQLException {
		...
		String queryParam = "('" + data.getAlarm_id() + "', '" + data.getNode_type() + "', '" + data.getNode_name()
				+ "', '" + data.getAlarm_occurance_time() + "', '" + data.getAlarm_severity() + "', ''," + " '', '', '"
				+ data.getAlarm_type() + "'," + " '" + data.getAlarm_cause() + "', '" + data.getCorrective_action()
				+ "'," + " ''," + 0 + "," + 0 + ", '" + data.getAlarm_state() + "','" + data.getNode_id()
				+ "')";
	...
	public void updateOrInsertRecords(List<Alarm> listOfData) throws SQLException {
		...
		if (existingAlarmList.isEmpty()) {
			...
			for (Alarm pollingAlarm : listOfData) {
				List<Alarm> matchedAlarm = existingAlarmList.stream()
						.filter(al -> al.getAlarm_id().equals(pollingAlarm.getAlarm_id())).collect(Collectors.toList());
				if (matchedAlarm == null || matchedAlarm.isEmpty()) {
					insertRecord(pollingAlarm);
				}
			}
			for (Alarm alarm : existingAlarmList) {
				long count = listOfData.stream().filter(al -> al.getAlarm_id().equals(alarm.getAlarm_id())).count();
				if (count == 0) {
					if (alarm.getAlarm_state().equals(Constants.ALARM_STATUS[0]))
						continue;
					Alarm alarm1 = new Alarm();
					alarm1.setAlarm_id(alarm.getAlarm_id());
					alarm1.setAlarm_state(Constants.ALARM_STATUS[0]);
					updateRecord(alarm1);
				}
			}
		}
